...
export default function CharacterGallery({ data, vision }) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (galleryContainer?.current)
        resizeObserver.unobserve(galleryContainer.current);
    };
  ...
  useEffect(() => {
    if (characterPics.length > 0) {
      ...
      const activeItemIndex = characterPics.findIndex(
        (item) => item.key === activePage.key,
      );
            ...
            {characterPics.map((item) => (
              ...
              >
                {item.key === activePage.key && (
                  <>
                    <div
                      className={`${styles.gallery_pic} ${item.key === "Card" ? styles.card : ""}`}
                    >
                      <Image
                        src={item.img}
                        width={450}
                    ...
                    <div className={styles.gallery_desc}>
                      <div
                        dangerouslySetInnerHTML={{ __html: item.imgDesc }}
                      ></div>
                    </div>
                  </>
                )}
